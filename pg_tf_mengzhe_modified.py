...
class PolicyModel:
  def __init__(self, D, K, hidden_layer_sizes):
    ...
    # at final layer, the output is probability of choosing each action
    ...
    # First only the entries of p_a_given_s that correspond to actions we actually took matter
    # i.e., p_a_t_given_s. 
    # so, another way is: p_a_t_given_s = p_a_given_s[index of actual action at time t]
...
class ValueModel:
  def __init__(self, D, hidden_layer_sizes):
    ...
    # final layer
    layer = HiddenLayer(M1, 1, lambda x: x) # final layer: no activation function
    self.layers.append(layer)
...
def play_one_mc(env, pmodel, vmodel, gamma):
  ...
  states_reversed = list(reversed(states))
  rewards_reversed = list(reversed(rewards))
  # for i in range(len(rewards)):
  #   returns.append(G)
  #   s = states_reversed[i]
  #   r = rewards_reversed[i]
  ...
  for i in range(len(rewards)):
    ...
    # at the end of an episode, G_T = reward_T
    if (i==0):
      # s_ = states_reversed[i-1]
      # r = rewards_reversed[i]
      G = r + gamma*G 
    else:
      s_ = states_reversed[i-1]
      G = r + gamma*vmodel.predict(s_)[0]
      # G = r + gamma*G
...
def main():
  ...
  pmodel = PolicyModel(D, K, [2])
  vmodel = ValueModel(D, [3])
